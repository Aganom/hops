 ...
 * {@link FsPermission} and the inode also contains an {@link InodeAcl}.  For
...
final class AclStorage {
  ...
  public static void copyINodeDefaultAcl(INode child) throws TransactionContextException, StorageException {
    ...
    InodeAcl parentInodeAcl = parent.getInodeAcl();
    if (parentInodeAcl == null || !(child.isFile() || child.isDirectory())) {
      return;
    }

    // Split parent's entries into access vs. default.
    List<AclEntry> featureEntries = parent.getInodeAcl().getEntries();
    ...
    if (!isMinimalAcl(accessEntries) || !defaultEntries.isEmpty()) {
      ...
      child.addInodeAcl(createInodeAcl(accessEntries, defaultEntries));
   ...
   * only the extended ACL entries stored in the InodeAcl.  If the inode does
   ...
   * {@link FsPermission} and {@link InodeAcl}.  Note that every inode
  ...
  public static void updateINodeAcl(INode inode, List<AclEntry> newAcl)
    ...
    if (!isMinimalAcl(newAcl)) {
      ...
      if (inode.getInodeAcl() != null) {
        inode.removeInodeAcl();
      }
      inode.addInodeAcl(createInodeAcl(accessEntries, defaultEntries));
  ...
  /**
   * Creates an InodeAcl from the given ACL entries.
   *
   * @param accessEntries List<AclEntry> access ACL entries
   * @param defaultEntries List<AclEntry> default ACL entries
   * @return InodeAcl containing the required ACL entries
   */
  private static InodeAcl createInodeAcl(List<AclEntry> accessEntries,
    ...
    return new InodeAcl(Collections.unmodifiableList(featureEntries));
  }
